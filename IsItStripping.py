...
with st.sidebar:
    ...
    Ropt = st.number_input("Optical radius R$_{\mathrm{opt}}$ [kpc]", value=1.175, min_value=0.01, step=0.05, format="%.3f", help='HI detections are usually unresolved, so the optical radius can be used to estimate the original HI extent in these cases')
    vrot = st.number_input("Rotation speed v$_{\mathrm{rot}}$ [km/s]", value=15.0, min_value=1.0, step=1.0, format="%.1f", help='Rotation speed after correcting the measurements for inclination and whatnot')
...
with col1:
    ...
    with st.expander("Inputs & intermediate values"):
        ...
        st.write(f"Σ<sub>0</sub> (pre-stripping central gas surface density) = **{prestripcentral:.3f}** M<sub>☉</sub>/pc²", unsafe_allow_html=True)        
